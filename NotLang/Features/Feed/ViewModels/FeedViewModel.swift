...
class FeedViewModel {
    ...
    var timeUntilNextDrop: String {
        ...
        guard let nextDropDate = utcCalendar.nextDate(after: now, matching: components, matchingPolicy: .nextTime) else {
            return "00:00:00"
        }
        
        let diff = utcCalendar.dateComponents([.hour, .minute, .second], from: now, to: nextDropDate)
